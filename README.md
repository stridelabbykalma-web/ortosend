...
- **Asistente de captura** guiado con guardado continuo: cuestionario clínico completo
  ...
  **núcleo de 5 tests obligatorios** (Jack/Hubscher, navicular drop, resistencia a la
  ...
  se adjunta). El **escaneo llega solo desde RevoScan al taller**: el puente instalado en el
  PC del escáner (`tools/puente-escaneo/`) sube cada mesh exportado directo al almacén
  común (Cloudflare R2, sin límite de tamaño) y se asocia al caso que está abierto en el
  paso del escaneo; con duda, la app pregunta «¿es de este paciente?». El taller lo descarga
  desde el expediente. Sin carpetas por paciente ni nombres que respetar. Token del puente
  por clínica en *Panel → Puente de escaneo*. Detalle y variables `R2_*` en
  `docs/puente-escaneo.md`.
...
- **Estudio de captura guiado con MediaPipe** (`src/components/caso/captura-studio.tsx`,
  ...
  **los dos pies de cerca** (talones y dedos detectados, llenando el encuadre, talones hacia la
  ...
  expediente reproduce el vídeo con los puntos superpuestos, y el apartado «Análisis de la
  marcha» de la exploración (que va después de los vídeos) se rellena solo con ese informe
  (patrón de pisada, ángulo de paso, retropié en apoyo y observaciones con las cifras); el
  profesional lo revisa y completa contacto inicial y despegue. Todo etiquetado como orientativo. La grabación es real (MediaRecorder) y la
