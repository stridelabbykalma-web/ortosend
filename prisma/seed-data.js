...
async function seedDemo(prisma) {
  ...
  await prisma.capture.create({
    data: {
      caseId: caso1.id,
      questionnaire: { motivo: "Dolor en talón derecho al levantarse, 3 meses. Trabaja 8 h de pie.", dolor: "7/10", actividad: "Activo" },
      physicalExam: { tobillo: "Limitada rodilla extendida (gastrocnemios)", hallux: "Normal", dismetria: "No", alza: "No" },
      completedAt: new Date(),
      media: {
  ...
  await prisma.capture.create({
    data: {
      caseId: caso2.id,
      questionnaire: { motivo: "Metatarsalgia bilateral, corredor habitual.", dolor: "6/10", actividad: "Deportista habitual" },
      physicalExam: { tobillo: "Normal", hallux: "Normal", dismetria: "No", alza: "No" },
      completedAt: new Date(),
      media: {
        create: ["scan_L", "scan_R", ...VIDEO_KINDS, ...BARO_KINDS].map((kind) => ({
          kind,
